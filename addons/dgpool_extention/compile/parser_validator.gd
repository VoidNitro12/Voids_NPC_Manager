...
func validate_dialogue_file(file_path: String) -> Dictionary:
	...
	for line in lines:
		...
		if bare_line.begins_with(EVENT_MARKER):
			to_skip = []
			valid = check_event_type(bare_line)
			stack_check.append(valid)
		elif bare_line.begins_with(VIBE_MARKER):
			if VIBE_MARKER in to_skip:
				continue
			else:
				to_skip = []
				valid = check_vibe(bare_line)
				stack_check.append(valid)
		elif bare_line.begins_with(MODE_MARKER):
			if MODE_MARKER in to_skip:
				continue
			else:
				to_skip = []
				valid = check_mode(bare_line)
				stack_check.append(valid)
		elif bare_line.begins_with(SECTION_MARKER):
			if SECTION_MARKER in to_skip:
				continue
			else:
				to_skip = []
				valid = check_section(bare_line)
				stack_check.append(valid)
		elif bare_line.begins_with(NPC_LINE_MARKER):
			if NPC_LINE_MARKER in to_skip:
				continue
			else:
				to_skip = []
				valid = check_npc_line(bare_line)
				stack_check.append(valid)
		elif bare_line.begins_with(RESPONSE_MARKER):
			if RESPONSE_MARKER in to_skip:
				continue
			else:
				to_skip = []
				valid = check_responses(bare_line)
				stack_check.append(valid)
		else:
			push_error("Unrecognized line beginning on line %d"%line_num)
			move_till_next_top()
			stack_check.append(false)
...
func check_event_type(line: String):
	current_event = null
	current_vibe = null
	current_mode = null
	current_section = null
	on_npc_line = false
	current_level = EVENT_MARKER
	var event_name = line.trim_prefix(EVENT_MARKER).strip_edges()
	if event_name not in NpcManager._event_types:
		push_error("Invalid Event Type on line %d, skipping. see NpcManager.get_event_types() " %line_num)
		move_till_next_top()
		return false
	else:
		current_level = VIBE_MARKER
		current_event = event_name
		locator["EVENT"][event_name] = line_num
		return true
		
func check_vibe(line: String):
	current_vibe = null
	if current_event != null:
		var name = line.trim_prefix(VIBE_MARKER).strip_edges()
		if name not in NpcDialogue.descriptor.keys():
			push_error("Invalid Emotional Descriptor on line %d, Skipping descriptor" %line_num)
			move_till_next_top()
		else:
			current_level = MODE_MARKER
			current_vibe = name
			var locator_name = current_event + "_" + name
			locator["VIBE"][locator_name] = line_num
			return true
	else:
		push_error("No valid Event type to place Descriptor under on line %d, Skipping descriptor"%line_num)
		move_till_next_top()
	return false

func check_mode(line: String):
	current_mode = null
	if current_vibe != null:
		var name = line.trim_prefix(MODE_MARKER).strip_edges()
		if name not in NpcDialogue.PoolContext.keys():
			push_error("Invalid Pool Context on line %d Skipping context" %line_num)
			move_till_next_top()
		else:
			current_level = SECTION_MARKER
			current_mode = name
			var locator_name = current_event + "_" + current_vibe + "_" + name
			locator["MODE"][locator_name] = line_num
			return true
	else:
		push_error("No valid Emotional Descriptor to place Context under on line %d, Skipping"%line_num)
		move_till_next_top()
	return false

func check_section(line: String):
	current_section = null
	if current_mode != null:
		var name = line.trim_prefix(SECTION_MARKER).strip_edges()
		current_section = name
		current_level = NPC_LINE_MARKER
		var locator_name = current_event + "_" + current_vibe + "_" + current_mode + "_" + name
		locator["SECTION"][locator_name] = line_num
		return true
	else:
		push_error("No valid Emotional Descriptor  to place Context under on line %d, Skipping"%line_num)
		move_till_next_top()
...
func check_npc_line(line: String):
	on_npc_line = false
	if current_section != null:
		var text = line.trim_prefix(NPC_LINE_MARKER).strip_edges()
		if not text.begins_with("\"") and not text.ends_with("\""):
			push_error("NPC line must be contained in qoutes with no other statements, line %d, Skipping"%line_num)
			move_till_next_top()
			return false
		else:
			current_level = RESPONSE_MARKER
			on_npc_line = true
			return true

func check_responses(line: String):
	if on_npc_line:
		...
		push_error("No NPC Line to put Pos Responses under, line %d"%line_num)
		move_till_next_top()
	return false
...
func validate_response_format(text: String) -> bool:
	...
	if quote_pos == -1:
		push_error("No opening quote, line %d"%line_num)
		return false
	if paren_pos == -1 or paren_pos < quote_pos:
		push_error("Metadata parentheses must come after quoted text, line %d"%line_num)
		return false
	if not text.ends_with(")"):
		push_error("Missing closing parenthesis, line %d"%line_num)
		return false
	return true

func move_till_next_top():
	var current = current_level
	to_skip = hierarchy[current]
